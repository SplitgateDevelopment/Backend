...
const ignoredPaths = ['DamageTypes', 'Elemental_Effect', 'EMPGrenade', 'Fists', 'Flag', 'FlagBase'];
...
(async () => {
    ...
    await writeFileSync(cosmeticsPath, JSON.stringify({
        ...oldData,
        ...output,
    }, null, 4));
...
async function updateSubsElements(elements, path) {
    ...
    await elements.reduce(async (pre, { name, subs }) => {
        await pre;
        if (!subs) return newElements.push({
            name,
        });
        
        const elementPath = join(path, name);
        if (!await exists(elementPath)) return log(`Sub path ${elementPath} does not exist`, 0);

        const data = readdirSync(elementPath).filter(filter);
        data.forEach(d => newElements.push({
            name: join(name, d),
        }));
    });
    return newElements;
...
async function pushToOutput(elements, path) {
    ...
    await elements.reduce(async (pre, { name }) => {
        ...
        const elementPath = join(path, name);
        if (!await exists(elementPath)) return log(`${name} element does not exist`, 0);
        ...
        if (!names.includes('Skins')) {
            ...
            await names
            .map(removeLowDash)
            .forEach(x => output[key].push(x.startsWith(`${el}_`) ? x : `${el}_${x}`));
