...
import { randomArrayElement, randomInt, randomString } from "../lib/utils";
...
const customizationPath = join(
	process.cwd(),
	"./assets/json/customization.json",
);
...
app.post("/:id/drops/open", async (c) => {
	...
	const keys = Object.keys(cosmetics).filter(
		(key) => !excludedDrops.includes(key),
	) as (keyof typeof cosmetics)[];
